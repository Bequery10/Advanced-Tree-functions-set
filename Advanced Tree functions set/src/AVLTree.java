...
public class AVLTree {
	...
	// levelTraversing();
	// System.out.println("rotateRight  data: "+root.data+"  height: "+root.height);
	...
	//levelTraversing();
	//System.out.println("rotateLeft  data: "+root.data+" height: "+root.height);
	...
	private Node balance(Node root, int data,String operation){
		if(root==null) return null;
		int leftHeight=0;
		int rightHeight=0;
		if(root.leftNode!=null) leftHeight=root.leftNode.height;
		if(root.rightNode!=null) rightHeight=root.rightNode.height;
		//System.out.println("balance  data: "+root.data+" rightHeight: "+rightHeight+" leftHeight: "+leftHeight);
		...
		else if(operation.equalsIgnoreCase("remove")){//----------------------remove
		...
		if(leftHeight-rightHeight>1){
			...
			int leftHeight2=0;
			int rightHeight2=0;
			if(node.leftNode!=null) leftHeight2=node.leftNode.height;
			if(node.rightNode!=null) rightHeight2=node.rightNode.height;
		
			if(leftHeight2==rightHeight2){
			   root=rotateRight(root);
		   }
		   else if(leftHeight2>rightHeight2) {
			   root=rotateRight(root);
		   } 
		   else {
			   root.leftNode=rotateLeft(root.leftNode);
		...
		else if(rightHeight2>leftHeight2) {
...
private Node insertHelper(Node root, Node node) {
		...
		if(root==null) {
			...
			root=balance(root,node.data,"insert");
...
public int getHeightField(Node node){
	return node.height;
}
